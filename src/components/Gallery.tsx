...
const GalleryImage = ({
	...
	return (
		...
		>
			<motion.div
				initial={{
					x: slideInInfo.current.x,
					y: slideInInfo.current.y,
					rotate: slideInInfo.current.rotate,
				}}
				animate={{
					x: show ? 0 : slideInInfo.current.x,
					y: show ? 0 : slideInInfo.current.y,
					rotate: show ? 0 : slideInInfo.current.rotate,
				}}
...
const Gallery = () => {
	...
	const handleImageClick = useCallback((thumb: Picture, full: string) => {
